...
operator fun Intent.set(key: String, value: Any?) {
    when (value) {
        ...
        is ArrayList<*> -> when (value.getOrNull(0)) {
            ...
            is CharSequence -> putCharSequenceArrayListExtra(
                key,
                value as java.util.ArrayList<CharSequence>
            )

            is String -> putStringArrayListExtra(key, value as java.util.ArrayList<String>)
            is Parcelable -> putParcelableArrayListExtra(
                key,
                value as java.util.ArrayList<Parcelable>
            )
        }
...
class IntentExtraGetter internal constructor(private val intent: Intent, val key: String) {
    ...
    fun <T : Parcelable> asParacelableList(): java.util.ArrayList<T>? =
        intent.getParcelableArrayListExtra<T>(key)

    fun asCharSequenceList(): java.util.ArrayList<CharSequence>? =
